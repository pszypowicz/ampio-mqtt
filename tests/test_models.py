...
def test_object_key_is_the_object_id() -> None:
    assert _object(id=150).object_key == "obj_150"


def test_object_key_separates_views_of_one_output() -> None:
    """Two Designer views of one output share a leaf but not an identity."""
    leaf = "0_be82_257_2_2"
    relay_view = _object(id=150, leaf_key=f"leaf_{leaf}")
    bell_view = _object(id=151, leaf_key=f"leaf_{leaf}")
...
def test_is_server_owned_reads_the_address_mac() -> None:
    server = ModuleAddress(mac=MSERV_MAC, channel=0, sf_id=257, sub_sf_id=0)
    assert _object(address=server).is_server_owned is True
    assert _object().is_server_owned is False
...
def test_the_alarm_halves_classify_on_the_address_sub_function() -> None:
    ...
    assert (
        _object(typ_komponentu="satel_alarm", address=armed).kind.key == "alarm_armed"
    )


def test_the_object_has_no_leaf_property() -> None:
    obj = _object()
    for name in (
        "leaf_id",
        "hidden",
