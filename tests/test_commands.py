...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda c: c.switch(64), b"/api/set/64/switch"),
        ...
        (
            lambda c: c.set_colors(50, 10, 20, 30, 40),
            b"/api/set/50/setColors/10/20/30/40",
        ),
        (lambda c: c.set_colors(50, 1, 2, 3), b"/api/set/50/setColors/1/2/3/0"),
        (lambda c: c.open(48), b"/api/set/48/open"),
        (lambda c: c.close(48), b"/api/set/48/close"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda c: c.set_colors(50, 0, 0, 0, 0), b"/api/set/50/setColors/0/0/0/0"),
        (
            lambda c: c.set_colors(50, 255, 255, 255, 255),
            b"/api/set/50/setColors/255/255/255/255",
        ),
        (lambda c: c.set_roller_pos(48, 0), b"/api/set/48/setRollerPos/0/101"),
        (
            lambda c: c.set_roller_pos(48, 100, lamella=100),
            b"/api/set/48/setRollerPos/100/100",
        ),
        (lambda c: c.set_event(1), b"/api/setEvent/1"),
        (lambda c: c.set_event(65535), b"/api/setEvent/65535"),
    ],
)
...
@pytest.mark.parametrize(
    "call",
    [lambda c: c.turn_on(50), lambda c: c.switch(50)],
)
async def test_turn_on_and_switch_on_rgbw_are_rejected(
    connected: tuple[AmpioClient, FakeBroker], call
) -> None:
...
async def test_stop_publishes_the_stop_verb(
    connected: tuple[AmpioClient, FakeBroker],
) -> None:
    client, broker = connected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "roleta",
            48,
            lambda c: c.set_roller_pos(48, 55, confirm=1.0),
...
async def test_panel_output_switch_inverts_the_held_state() -> None:
    client, broker = await _admin_with_panel_output()
    try:
        await client.switch(90)  # no value yet -> reads off -> turns on
        feed(client, "ampio/from/CAFE/state/o/2", "1")
        await client.switch(90)
        assert broker.published == [
            (PANEL_RAW_TOPIC, b"30f9ff01"),
            (PANEL_RAW_TOPIC, b"30f90001"),
        ]
