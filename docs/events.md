...
| `NotConfigured`       | The catalogue lists rows without a leaf. Carries the `(id, name)` pairs. The rows stay out of `objects` until a later catalogue restores the leaf.                                                                                       | both       | no       |
| `PresenceChanged`     | The presence-detection or presence-simulation row appeared, left, or changed. Carries both rows as they read after the change (see [`presence.md`](presence.md)).                                                                        | both       | no       |
