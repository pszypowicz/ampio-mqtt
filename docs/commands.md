...
read-only object, or a command that changed nothing. Latency bounds the timeout
choice. Most verbs echo in under ~200 ms on the per-object path, and
`arm`/`disarm` take ~1 s, so `confirm=2.0` covers the measured surface. Scene
commands and `setEvent` fan out beyond a single object and offer no per-object
echo.
...
| ---------------------------------- | ------------------------------------------ | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `turnOn`                           | `turn_on()`                                | -                           | Full on (255). Flags answer it too. `rgbw` objects ignore it (no effect, no reply) - see `setColors`.                                                                                                                                                                                                                   |
| `turnOff`                          | `turn_off()`                               | -                           | Off. Flags answer it too. `rgbw` objects ignore it (no effect, no reply) - turn those off with `setColors 0/0/0/0`.                                                                                                                                                                                                     |
| `switch`                           | `switch()`                                 | -                           | Inverts the current state. Flags answer it too. `rgbw` objects ignore it (no effect, no reply).                                                                                                                                                                                                                         |
| `open`                             | `open()`                                   | -                           | Cover to 100.                                                                                                                                                                                                                                                                                                           |
| `close`                            | `close()`                                  | -                           | Cover to 0.                                                                                                                                                                                                                                                                                                             |
| `stop`                             | `stop()`                                   | -                           | Halts a cover on either axis. Mid-travel, the position stream freezes at the halt point, and the target is never reached. A slat rotation caught mid-turn freezes at an intermediate angle too. In the pre-travel slat phase it cancels the pending move. Stationary, it is a silent no-op.                             |
| `setValue`                         | `set_value()`                              | `<0-255>[/<time>]`          | `time` is in 10 ms units and **reverts** the object afterwards - a timed pulse, not a fade. `rgbw` objects ignore it, with or without `time` (no effect, no reply) - see `setColors`.                                                                                                                                   |
| `setColors`                        | `set_colors()`                             | `<R>/<G>/<B>/<W>`           | Also accepts one packed int (`R \| G<<8 \| B<<16 \| W<<24`), which is what object state reports back. Absent from the spec enum but real. A fifth argument (a `time`, `setValue`-style) makes the M-SERV drop the whole command (no effect, no reply).                                                                  |
| `setRollerPos`                     | `set_roller_pos()`, `set_roller_lamella()` | `<position>/<lamella>`      | Percent each. `101` omits an axis (see the slat-drag note below), so one command moves either axis alone or both together.                                                                                                                                                                                              |
...
| `arm`, `disarm`                    | `command()`                                | `<pin>`                     | Flip a `satel_alarm` object's armed state, with a ~1 s echo. The `satel_` types cover alarm integrations, including a Jablotron behind an M-CON. Absent from the spec enum, yet it works. The paired "alarmed" object also reads 1 during the panel's exit-delay `arming` phase, but alone it is not a siren indicator. |
...
outputs but does not honor it. With transition times of 0, 5, and 20 seconds,
the bridge emits an identical CAN frame sequence. The output reaches its new
level in about 0.3 seconds, with no intermediate steps in the state stream. The
bridge also emits a ten-frame command where an `/api` `setValue` emits one
frame.
...
A position move on a blind drags its slats mechanically, and `101` means "send
no angle", not "hold the angle". The slats end wherever the travel leaves them:
closed (`lammel` 0) after a downward move, open (100) after an upward one. To
land on a chosen angle, pass an explicit `lamella` in the same command.
