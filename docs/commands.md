...
| `setValue`                         | `set_value()`                              | `<0-255>[/<time>]`          | `time` is in 10 ms units and **reverts** the object afterwards - a timed pulse, not a fade. `rgbw` objects ignore it, with or without `time` (no effect, no reply) - see `setColors`. A `ledww` ignores it too - its power axis moves through `setWWPower` alone.                                                                         |
