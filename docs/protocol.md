...
| [`panel-writes.md`](panel-writes.md)           | The raw CAN output frame for panel status LEDs, relays, and open-collector outputs, the panel buzzer, the touch field colors, the touch lock, module identify, and the cover roller lock. |
...
| `devices`        | `ampio/control/<user>/data`   | `ampio/fromDB/<user>/data/devices`        | `{List: [...]}` - the object catalogue, served to every account. A standard account sees its app grants. The admin login sees every object in a room. Every reply also carries the two system rows, and the library drops them by their type.                                                                                                                                                                                                                                           |
| `params_devices` | `ampio/control/<user>/data`   | `ampio/fromDB/<user>/data/params_devices` | `{List: [{id, params, param1, czas, powiazane, url}]}` - per-object `params` bitfields for the **full** catalogue (not grant-filtered). `powiazane` lists linked objects as `<linkId>:<objectId>` pairs, comma separated, and reads null when none. The `logging` payload on the same request topic is `{List: [{id_obiektu, typ_logowania}]}`, one row per object that Designer's "Log data" box covers, and `typ_logowania` is the interval in seconds, with 0 for "logged in turns". |
...
| `scenes`         | `ampio/control/<user>/data`   | `ampio/fromDB/<user>/data/scenes`         | `{List: [{id, parentId, sceneName, active, Actions, Infos, Schedules}]}` - scene catalogue.                                                                                                                                                                                                                                                                                                                                                                                             |
...
| (empty)          | `ampio/control/<user>/info`   | `ampio/fromDB/<user>/data/info`           | `{Results: {mac, userId, serverVersion, serverRevision, mqttVersion, local_ip, device_id, ...}}` - server self-report, retained in the account namespace. `userId` is the asking account's id (`-1` for the reserved `admin` login). The client class decides what a running client requests. `AmpioServerInfo.user_id` carries the account id, and `access_tier` derives the tier from it. A config flow can read the tier to pick a class.                                            |
...
A column can hold an empty value. `leafId`, `opis_menu` on an object row, and
`format` are text columns the M-SERV writes empty, or null, when the object
carries no value. A null `url` refuses the `params_devices` reply, and only an
empty string is accepted. The `type` column is null or empty on an untagged
object. What must be there is the column itself. A `leafId` that is neither a
string nor null refuses the reply.
...
`states`, so the library saves no request with them. The admin client reads
`md5/devices` and `md5/params_devices` as the signal to re-request the module
list, which the M-SERV never pushes. The Designer-save push and the re-request
rule are in [`discovery-flow.md`](discovery-flow.md).

## Live state

| Topic                                  | Payload                           | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ampio/fromDB/<user>/ob/<id>/state`    | `{state, desc, on}`               | One per object. `state` is the value (string), `desc` is the M-SERV's pretty form, `on` is server-side ms epoch. Cover (`roleta*`) pushes carry a `block` field in place of `desc`, exposed as `AmpioObject.block`. Regulator (`reg`) objects push a richer shape instead: `{state, cooling, mode, measureTemp, setTemperature, on}`, every field but `on` a string, exposed as `AmpioObject.thermostat`. The library exposes `state`, `lammel`, `block`, and the reg readback from these. |
...
| `ampio/from/<MAC>/state/a/<ch>`        | plain text (u8, `"0"` to `"255"`) | Analog output channel, subscribed on `AmpioAdminClient` and bridged to the owning `przekaznik` object on an open-collector leaf (class 67). Every other `a` channel drops at the index lookup.                                                                                                                                                                                                                                                                                             |
