...
| Method                       | What it does                                                                                                                                                                                                                                                                                               |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AmpioClient.start()`        | Connects, subscribes, publishes the tier's four auto-discovery requests (admin: `devicesDetails` + `devices` on `config`; standard: `devices` + `params_devices` on `data`; both: the empty-payload `states` and `info` surfaces), waits for the tier's catalogue set, returns. See [`discovery-flow.md`](discovery-flow.md). |
