...
| (empty)          | `ampio/control/<user>/info`   | `ampio/fromDB/<user>/data/info`           | `{Results: {mac, userId, serverVersion, serverRevision, mqttVersion, local_ip, device_id, ...}}` - server self-report, retained in the account namespace. `userId` is the asking account's id (`-1` for the reserved `admin` login). The client class decides what a running client requests. `AmpioServerInfo.access_tier` reports the account id as a wire fact, and a config flow can read it to pick a class.                                                                       |
