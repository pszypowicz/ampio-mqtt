...
| `setHeatingMode`                   | mode letter                 | `M` switched a regulator from Schedule to Manual (state push `mode` went `S` -> `M`); sending `S` back was silently ignored, so only `M` writes of the claimed `A,S,M,H`. Readback vocabulary: `S` and `M` live-proven by that transition, `A` observed in a live `stan_json`, `H` spec-only - `ThermostatState.mode` carries the letter verbatim.        |
...
Which `AmpioClient` method drives which surface is API documentation and
lives on the client docstrings; [`discovery-flow.md`](discovery-flow.md)
maps the automatic bring-up sequence against the on-demand fetches.
