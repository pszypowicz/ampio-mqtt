...
| `params_devices` | `ampio/control/<user>/data`   | `ampio/fromDB/<user>/data/params_devices`   | `{List: [{id, params, param1, czas, powiazane, url}]}` - per-object `params` bitfields for the **full** catalogue (not grant-filtered). `powiazane` lists linked objects as `<linkId>:<objectId>` pairs, comma separated, and reads null when none.                                                                                                               |
